...
export function detectIntent(question, category) {
  ...
  const intentMap = { '感情': ['复合','分手','前任','脱单','正缘','桃花','暧昧','他爱','出轨','婚姻','结婚','离婚','心动'], '财运': ['财运','赚钱','项目','投资','破财','工资','偏财','奖金','股票','基金'], '事业': ['工作','跳槽','升职','面试','创业','辞职','老板','同事','裁员'], '健康': ['身体','生病','手术','失眠','焦虑','抑郁','头疼'], '学业': ['考试','考研','考公','成绩','论文','上岸','毕业'], '人际关系': ['小人','贵人','朋友','婆媳','婆婆','媳妇','社交'], '决策': ['该不该','选哪个','要不要','能不能','怎么办','纠结'] };
...
export async function localInterpretation() {
  ...
  for (const g of allGongs) {
    ...
    cards.forEach(card => {
      ...
      const ctx = { gong: { id: g, name: GONG_NAMES[g], element: GONG_WUXING[g] }, card: { element: getWuxing(card), value: getCardValue(card), suit: card.suit }, tiYongRelation: relToTi || '同我', wangState: wang, linePosition, diff, intent };
  ...
  if (durian) { state.durianIndex = durian; result += `\n🍈 榴莲指数：${durian.score}/10（${durian.level}）\n${durian.description}\n`; }
...
export async function generateInterpretation() {
  ...
  const metaphor = generateMetaphor({
    tiYongRelation: getShengKe(getWuxing(state.ti), getWuxing(state.yong)) || '同我',
  ...
  updateStep(2); // 保持在布阵步骤，但已经出结果了，这里维持步骤条不变（仍在第二步），避免跳转混乱
...
export function showFullReport() {
  ...
  updateStep(3);
  try {
    const readingData = { time: Date.now(), question: state.question, category: state.category, intent: state.intent, ti: state.ti, yong: state.yong, grid: state.grid, line: state.line, lineOrder: state.lineOrder, text, chatHistory: state.chatHistory.slice(), durianScore: state.durianIndex?.score || 0 };
...
export async function buildAIPrompt() {
  ...
  const profile = getProfile();
  let personalPrefix = '';
  if (profile) { let parts = []; if (profile.name) parts.push(`姓名：${profile.name}`); if (profile.gender) parts.push(`性别：${profile.gender}`); if (profile.birthPlace) parts.push(`出生地：${profile.birthPlace}`); if (profile.currentPlace) parts.push(`现居地：${profile.currentPlace}`); if (parts.length) personalPrefix = '【求测人信息】' + parts.join('，') + '\n\n'; }
...
export function switchMode(mode) { if (!Object.values(MODES).includes(mode)) return; state.mode = mode; refreshAll(); }
...
export function handleAction(action, dataset) {
  switch (action) {
    ...
    case 'selectCategory': state.category = state.category === dataset.category ? '' : dataset.category; document.querySelectorAll('[data-action="selectCategory"]').forEach(b => b.classList.toggle('selected', b.dataset.category === state.category)); break;
