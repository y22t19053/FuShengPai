// ===== 浮生牌 · UI 层 =====
// DOM 渲染、事件委托、拖拽系统、动画控制、多步引导、实体教学。
...
function togglePanel(panelId) {
  ...
  $$('.panel').forEach(p => p.setAttribute('hidden', ''));
  if (isHidden) {
...
function generateInterpretation() {
  ...
  try {
    saveReading({ time: Date.now(), question: state.question, category: state.category, ti: state.ti, yong: state.yong, grid: state.grid, line: state.line, lineOrder: state.lineOrder, text });
...
function updateBaziPreview() {
  ...
  try {
    ...
    preview.textContent = '日期无效，请检查输入';
  }
}
...
function renderHistoryPanel() {
  const list = $('#historyList'); if (!list) return; const history = getHistory();
...
function showHistoryDetail(index) {
  const r = getHistory()[index]; if (!r) return;
  domModalContent.innerHTML = `<h3>历史详情</h3><p><strong>时间：</strong>${new Date(r.time).toLocaleString()}</p><p><strong>问题：</strong>${r.question || '未提问'}</p><p><strong>类别：</strong>${r.category || '无'}</p><div class="result-block">${(r.text || '').replace(/\n/g, '<br>')}</div><div class="btn-row"><button data-action="deleteHistoryItem" data-history-index="${index}" class="outline small">删除此条</button><button data-action="closeModal" class="small">关闭</button></div>`;
...
async function triggerAI() {
  ...
  try {
    ...
    const followUp = $('#followUpArea'); if (followUp) followUp.style.display = 'block'; toast('解读完成');
...
async function sendFollowUp() {
  ...
  try { const result = await requestFollowUp({ history, provider: settings.provider || 'deepseek', apiKey: settings.apiKey, endpoint: settings.endpoint, model: settings.model }); history.push({ role: 'assistant', content: result }); if (chatBlock) { chatBlock.innerHTML += `<div class="chat-msg ai">${result.replace(/\n/g, '<br>')}</div>`; chatBlock.scrollTop = chatBlock.scrollHeight; } } catch (e) { if (chatBlock) chatBlock.innerHTML += `<div class="chat-msg" style="color:#d45050">失败：${e.message}</div>`; }
}

function buildAIPrompt() { return `请根据以下浮生牌局象进行详细解读。\n\n要求：纯文本格式，严禁使用任何Markdown符号。用自然语言分段。从体用生克、时空推演、宫位差值、旺相休囚、阴阳属性等方面展开。话不说死。\n\n${localInterpretation()}\n\n规则：红桃火(阳) 方块金(阳) 梅花木(阴) 黑桃水(阴) J/Q/K土 大王天(阳) 小王人(阴)。`; }
...
// ===== 拖拽系统 =====
const DRAG_THRESHOLD = 4; let dragData = null;
function startDrag(clientX, clientY, cardEl) { const id = cardEl.dataset.cardid; const card = findCardById(id); if (!card || isCardPlaced(card)) return; const rect = cardEl.getBoundingClientRect(); dragData = { cardId: id, cardEl, startX: clientX, startY: clientY, offsetX: clientX - rect.left, offsetY: clientY - rect.top, origRect: rect, moved: false, clone: null }; }
function moveDrag(clientX, clientY, e) { if (!dragData) return; const dx = clientX - dragData.startX; const dy = clientY - dragData.startY; if (!dragData.clone && (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD)) { if (e && e.preventDefault) e.preventDefault(); const clone = dragData.cardEl.cloneNode(true); clone.style.position = 'fixed'; clone.style.zIndex = '10000'; clone.style.opacity = '0.9'; clone.style.pointerEvents = 'none'; clone.style.left = dragData.origRect.left + 'px'; clone.style.top = dragData.origRect.top + 'px'; clone.style.width = dragData.origRect.width + 'px'; clone.style.height = dragData.origRect.height + 'px'; clone.style.transition = 'none'; document.body.appendChild(clone); dragData.clone = clone; dragData.moved = true; } if (dragData.clone) { if (e && e.preventDefault) e.preventDefault(); dragData.clone.style.left = (clientX - dragData.offsetX) + 'px'; dragData.clone.style.top = (clientY - dragData.offsetY) + 'px'; } }
function endDrag(clientX, clientY) { if (!dragData) return; if (dragData.clone && dragData.moved) { const clone = dragData.clone; const card = findCardById(dragData.cardId); clone.style.display = 'none'; const target = document.elementFromPoint(clientX, clientY); clone.style.display = ''; let placed = false; if (card && !isCardPlaced(card)) { const tg = target?.closest('.gong'); if (tg) placed = placeCardOnGong(card, parseInt(tg.dataset.gong)); const td = target?.closest('.empty-dash'); if (!placed && td) { if (td.textContent.includes('体')) placed = placeCardOnTiYong(card, 'ti'); else if (td.textContent.includes('用')) placed = placeCardOnTiYong(card, 'yong'); } } if (placed) clone.remove(); else { clone.style.transition = 'left 0.2s ease, top 0.2s ease'; clone.style.left = dragData.origRect.left + 'px'; clone.style.top = dragData.origRect.top + 'px'; const onEnd = () => { clone.remove(); clone.removeEventListener('transitionend', onEnd); }; clone.addEventListener('transitionend', onEnd); setTimeout(() => { if (clone.parentNode) clone.remove(); }, 300); } } else selectCard(dragData.cardId); dragData = null; }

document.addEventListener('touchstart', function(e) { const cardEl = e.target.closest('.card-back, .card-face-small'); if (!cardEl) return; e.preventDefault(); startDrag(e.touches[0].clientX, e.touches[0].clientY, cardEl); }, { passive: false });
document.addEventListener('touchmove', function(e) { if (!dragData) return; moveDrag(e.touches[0].clientX, e.touches[0].clientY, e); }, { passive: false });
document.addEventListener('touchend', function(e) { if (!dragData) return; endDrag(e.changedTouches[0].clientX, e.changedTouches[0].clientY); });
document.addEventListener('mousedown', function(e) { const cardEl = e.target.closest('.card-back, .card-face-small'); if (!cardEl) return; e.preventDefault(); startDrag(e.clientX, e.clientY, cardEl); });
document.addEventListener('mousemove', function(e) { if (!dragData) return; moveDrag(e.clientX, e.clientY, e); });
document.addEventListener('mouseup', function(e) { if (!dragData) return; endDrag(e.clientX, e.clientY); });

// ===== 重置 =====
function resetAll() { Object.assign(state, { question: '', category: '', deck: [], ti: null, yong: null, grid: {}, line: null, lineOrder: {}, step: 1, sel: null, possible: [], manualMode: false, gongOrder: [], chatHistory: [], uid: 0, editCount: 0 }); updateStep(1); renderStep1(); toast(UI_TEXTS.toastReset); }

function startQuestion() { const h = new Date().getHours(); if (h >= 23 || h < 1) { domDynamic.innerHTML = `<div class="panel"><h3>子时不卜</h3><p style="text-align:center;padding:30px">${TIME_RESTRICTION.message}</p></div>`; return; } const q = $('#questionInput')?.value?.trim() || ''; if (q) { const check = checkEthicalBoundary(q); if (check.blocked) { domDynamic.innerHTML = `<div class="panel"><h3>提示</h3><p>${check.message}</p><button data-action="resetAll" class="small">返回</button></div>`; return; } } state.question = q; state.manualMode = false; state.uid = 0; state.deck = shuffle(createDeck(false)); state.ti = null; state.yong = null; state.grid = {}; state.line = null; state.lineOrder = {}; state.sel = null; state.possible = []; state.chatHistory = []; state.gongOrder = []; state.editCount = 0; updateStep(2); renderStep2(); }
function startManualEntry() { const h = new Date().getHours(); if (h >= 23 || h < 1) { toast(TIME_RESTRICTION.message); return; } state.question = $('#questionInput')?.value?.trim() || ''; state.manualMode = true; state.uid = 0; state.deck = createDeck(true); state.ti = null; state.yong = null; state.grid = {}; state.line = null; state.lineOrder = {}; state.sel = null; state.possible = []; state.chatHistory = []; state.gongOrder = []; state.editCount = 0; updateStep(2); renderStep2(); }
...
function saveApiSettingsFromForm() { const p = state.selectedProvider || 'deepseek'; const info = API_PROVIDERS[p] || API_PROVIDERS.deepseek; const settings = { provider: p, apiKey: $('#apiKey')?.value?.trim() || '', endpoint: $('#apiEndpoint')?.value?.trim() || info.endpoint || '', model: info.model || '', aiStyle: $('#aiStyle')?.value || 'guide' }; saveApiSettings(settings); updateApiStatus(); toast(UI_TEXTS.toastSaved); }
