// ===== 浮生牌 · 存储层 =====
// 所有持久化操作封装在此，其他模块不直接操作 localStorage。
...
export function getHistory() {
  try {
    ...
    console.warn('读取历史记录失败，将返回空数组', e);
    return [];
  }
}
...
export function saveReading(record) {
  ...
  try {
    ...
    console.error('保存历史记录失败：存储空间可能已满', e);
...
/**
 * 获取用户个人信息
 * @returns {{ birthDate: string, birthTime: string }}
 */
...
/**
 * 保存个人信息
 * @param {{ birthDate: string, birthTime: string }} profile
 */
    ...
    reader.onload = (e) => {
      try {
        ...
        for (const record of data.history) {
          if (existingTimes.has(record.time)) {
            skipped++;
          } else {
...
export function clearAllData() {
  ...
  localStorage.removeItem(KEYS.DRAW_TIMESTAMPS);
}